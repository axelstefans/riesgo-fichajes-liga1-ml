...
def cargar_fichajes_entrada(filepath: str) -> pd.DataFrame:
    try:
        ...
        if missing_cols:
            raise ValueError(
                f"El archivo de entrada '{filepath}' no contiene las columnas requeridas: {missing_cols}"
            )
...
def cargar_progreso_previo(filepath: str) -> tuple[pd.DataFrame, list]:
    ...
    try:
        ...
        df_existente = pd.read_csv(
            filepath,
            sep=Config.CSV_SEPARATOR,
            dtype={"tm_id": str, "ss_id": str, "ss_season_id": str},
        )

        required_cols = ["tm_id", "ss_season_id"]
        if not all(col in df_existente.columns for col in required_cols):
            logger.warning(
                "Archivo de progreso previo tiene formato incorrecto. Se comenzará desde cero."
            )
            return pd.DataFrame(), []
        ...
        df_existente["unique_row_id"] = (
            df_existente["tm_id"].astype(str)
            + "_"
            + df_existente["ss_season_id"].astype(str)
        )
        procesados = df_existente["unique_row_id"].unique().tolist()
        return df_existente, procesados
    except Exception as e:
        logger.warning(
            f"No se pudo leer el archivo de resultados previo ({e}). Se comenzará desde cero."
        )
        return pd.DataFrame(), []
...
def filtrar_registros_pendientes(df: pd.DataFrame, procesados: list) -> pd.DataFrame:
    registros_invalidos = (
        df[["tm_id", "ss_id", "ss_season_id", "ss_tournament_id"]]
        .isna()
        .any(axis=1)
        .sum()
    )
    if registros_invalidos > 0:
        logger.warning(
            f"Se descartarán {registros_invalidos} registros con IDs faltantes."
        )

    df_validos = df.dropna(
        subset=["tm_id", "ss_id", "ss_season_id", "ss_tournament_id"]
    ).copy()
    df_validos["unique_row_id"] = (
        df_validos["tm_id"].astype(str) + "_" + df_validos["ss_season_id"].astype(str)
    )
    ...
    if procesados:
        ...
        logger.info(
            f"Se encontraron {registros_ya_procesados} registros ya procesados. Procesando {len(df_pendientes)} restantes."
        )
    else:
...
def procesar_jugadores(df_pendientes: pd.DataFrame, anio_fichaje: int) -> list:
    ...
    logger.info(
        f"Tiempo estimado de procesamiento para esta temporada: ~{tiempo_estimado_min:.1f} minutos."
    )

    datos_nuevos = []

    for idx, (_, jugador) in enumerate(
        tqdm(
            df_pendientes.iterrows(),
            total=total,
            desc=f"Procesando Fichajes {anio_fichaje}",
        )
    ):
        try:
            ...
            datos_jugador = obtener_datos_completos_jugador(
                tm_player_id=tm_id,
                tm_target_club=jugador["club_destino"],
                ss_player_id=ss_id,
                ss_season_id=ss_season,
                ss_tournament_id=ss_tournament,
            ...
            if datos_jugador:
                ...
                logger.info(
                    f"ÉXITO: Datos de '{datos_jugador.get('nombre_jugador', 'N/A')}' extraídos."
                )
        except Exception as e:
            logger.error(
                f"Error procesando TM_ID {jugador.get('tm_id', 'N/A')}: {e}",
                exc_info=True,
            )

        if idx < total - 1:
            pause_duration = random.uniform(
                Config.SCRAPING_MIN_DELAY, Config.SCRAPING_MAX_DELAY
            )
...
def guardar_resultados(
    datos_nuevos: list, df_existente: pd.DataFrame, output_path: str
):
    if not datos_nuevos:
...
if __name__ == "__main__":
    ...
    try:
        for temp_key, temp_config in Config.TEMPORADAS.items():
            ...
            sufijo_archivo_entrada = (
                f"{temp_config['tm_saison_id']}_{int(temp_config['tm_saison_id']) + 1}"
            )
            temp_config["input_file"] = (
                f"{Config.DIR_ENTRADA}/fichajes_{sufijo_archivo_entrada}.csv"
            )
            temp_config["output_file"] = (
                f"{Config.DIR_SALIDA_BRUTOS}/fichajes_brutos_{temp_config['file_suffix']}.csv"
            )
            ...
            try:
                ...
                logger.error(
                    f"Fallo irrecuperable en la temporada {temp_key}: {e}",
                    exc_info=True,
                )
            ...
            if temp_key != list(Config.TEMPORADAS.items())[-1][0]:
                ...
                logger.info(
                    f"Pausa de {pausa_entre_temporadas:.1f}s antes de la siguiente temporada."
                )
    ...
    for r in resumen_temporadas:
        logger.info(
            f"{r['temporada']:<15} {r['estado']:<25} {r.get('registros_extraidos', 'N/A'):<25}"
        )
    logger.info("=" * 80)
