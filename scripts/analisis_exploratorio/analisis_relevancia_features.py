...
def analizar_feature_importance(model, feature_names, threshold=0.005):
    ...
    irrelevant = df_importance[df_importance["importance"] < threshold][
        "feature"
    ].tolist()

    logger.info(f"✅ Total features: {len(feature_names)}")
    logger.info(
        f"⚠️ Features con importancia < {threshold * 100:.1f}%: {len(irrelevant)}"
    )
    ...
    if irrelevant:
        ...
        for feat in irrelevant:
            imp_pct = df_importance[df_importance["feature"] == feat][
                "importance_pct"
            ].values[0]
...
def analizar_varianza_baja(df, feature_names, threshold=0.01):
    ...
    df_variance = pd.DataFrame(
        {"feature": list(variances.keys()), "variance": list(variances.values())}
...
def analizar_correlacion_alta(df, feature_names, threshold=0.90):
        ...
        for j in range(i + 1, len(corr_matrix.columns)):
            ...
            if corr_val > threshold:
                ...
                high_corr_pairs.append(
                    {"feature1": feat1, "feature2": feat2, "correlation": corr_val}
                )
    ...
    logger.info(
        f"⚠️ Pares con correlación > {threshold * 100:.0f}%: {len(high_corr_pairs)}"
    )
    ...
    if high_corr_pairs:
        ...
        for pair in high_corr_pairs:
            logger.info(
                f"   {pair['feature1']:30s} ↔ {pair['feature2']:30s} | r = {pair['correlation']:.3f}"
            )
...
def analizar_features_sparse(df, feature_names, threshold=0.90):
    ...
    logger.info(
        f"⚠️ Features con > {threshold * 100:.0f}% ceros: {len(sparse_features)}"
    )
...
def generar_recomendaciones(results_dict):
    ...
    logger.info("\n📊 RESUMEN:")
    logger.info(
        f"   Total features originales: {results_dict.get('total_features', 'N/A')}"
    )
    logger.info(f"   Features problemáticas identificadas: {len(all_problematic)}")
    logger.info(
        f"   Features recomendadas a MANTENER: {results_dict.get('total_features', 42) - len(all_problematic)}"
    )

    logger.info("\n🚨 FEATURES RECOMENDADAS PARA ELIMINACIÓN:")
    logger.info("   (Ordenadas por número de problemas detectados)\n")
    for idx, row in df_summary.iterrows():
        logger.info(
            f"   [{row['num_razones']} problemas] {row['feature']:35s} → {row['razones']}"
        )

    logger.info("\n💡 RECOMENDACIÓN FINAL:")
    logger.info(f"   Eliminar las {len(all_problematic)} features listadas arriba")
    logger.info(
        f"   Nuevo total de features: {results_dict.get('total_features', 42) - len(all_problematic)}"
    )
    logger.info(
        f"   Ratio registros/features: {896 / (results_dict.get('total_features', 42) - len(all_problematic)):.1f}"
    )
...
def generar_visualizaciones(results_dict, output_dir):
    ...
    if "importance" in results_dict:
        ...
        colors = [
            "red" if x < threshold else "steelblue" for x in df_plot["importance"]
        ]

        ax.barh(
            range(len(df_plot)),
            df_plot["importance_pct"],
            color=colors,
            edgecolor="black",
        )
        ...
        plt.close()
        logger.info("   ✅ Guardado: 01_feature_importance.png")
    ...
    if "correlation" in results_dict:
        ...
        ax.set_title(
            "Matriz de Correlación (Top 20 Features)\nVerde = Positiva | Rojo = Negativa",
            fontweight="bold",
            fontsize=14,
        )
        plt.tight_layout()
        ...
        plt.close()
        logger.info("   ✅ Guardado: 02_correlation_heatmap.png")
    ...
    if "sparsity" in results_dict:
        ...
        colors = [
            "red" if x > threshold else "steelblue" for x in df_plot["zero_percentage"]
        ]

        fig, ax = plt.subplots(figsize=(12, 8))
        ax.barh(
            range(len(df_plot)),
            df_plot["zero_percentage"] * 100,
            color=colors,
            edgecolor="black",
        )
        ax.set_yticks(range(len(df_plot)))
        ...
        plt.close()
        logger.info("   ✅ Guardado: 03_sparsity_analysis.png")
...
def main():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default="datos_salida/finales/dataset_etiquetado.csv",
        help="Ruta al dataset etiquetado",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="model_artifacts/lgbm_model.joblib",
        help="Ruta al modelo entrenado",
    )
    parser.add_argument(
        "--metadata",
        type=str,
        default="model_artifacts/model_metadata.json",
    ...
    # 1. Feature Importance
    results["importance"] = analizar_feature_importance(
        model, feature_names, threshold=0.005
    )
    ...
    # 3. Correlación
    results["correlation"] = analizar_correlacion_alta(
        df, feature_names, threshold=0.90
    )
    ...
    recommendations["df_summary"].to_csv(
        output_dir / "feature_problems_summary.csv", index=False
    )

    logger.info("\n✅ ANÁLISIS COMPLETADO")
    logger.info(f"   📁 Reportes guardados en: {output_dir}")
    logger.info("   📄 feature_analysis_report.json")
    logger.info("   📄 features_to_drop.txt")
    logger.info("   📄 feature_problems_summary.csv")
    logger.info("   🖼️ 3 visualizaciones (.png)")
