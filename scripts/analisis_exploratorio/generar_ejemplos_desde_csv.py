...
def _obtener_datos_tm_simplificado(tm_player_id: str) -> dict | None:
    try:
        ...
        edad = (
            int(birth_info.split("(")[1].replace(")", "").strip())
            if birth_info and "(" in birth_info
            else 0
        )
        ...
        if not all([nationality_str, player_position, edad > 0]):
            raise ValueError(
                "No se pudieron extraer todos los datos biográficos de Transfermarkt."
            )
        ...
        logger.error(
            f"Error en Transfermarkt (simplificado) para TM_ID {tm_player_id}: {e}"
        )
        return None
...
def agregar_stats_por_ejemplo(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_dict.pop("tm_id", None)
    agg_dict.pop("anio_evaluacion", None)
...
def generar_json_ejemplos():
    ...
    try:
        df_input = pd.read_csv(ARCHIVO_ENTRADA, sep=Config.CSV_SEPARATOR, dtype=str)
        if "anio_evaluacion" not in df_input.columns:
            raise ValueError(
                "El archivo de entrada debe contener la columna 'anio_evaluacion'."
            )
    except Exception as e:
    ...
    try:
        for _, jugador in tqdm(
            df_input.iterrows(), total=len(df_input), desc="Extrayendo datos"
        ):
            try:
                ...
                if not ss_data:
                    logger.warning(
                        f"  - ⚠️ Sin datos SofaScore para SS_ID {jugador['ss_id']}"
                    )
                    continue

                registro_completo = {**jugador.to_dict(), **ss_data}

                tm_data = _obtener_datos_tm_simplificado(
                    str(jugador["tm_id"]).split(".")[0]
                )
                if tm_data:
                ...
                logger.error(
                    f"  - ❌ Error procesando SS_ID {jugador.get('ss_id', 'N/A')}: {e}"
                )
