...
def main():
    ...
    for col in cols_analisis:
        ...
        resultados.append({"Feature": col, "Ceros": num_ceros, "Porcentaje": pct_ceros})
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 50 + "\n")
        f.write(
            df_res.to_string(index=False, formatters={"Porcentaje": "{:.1f}%".format})
        )
    ...
    if top_val > 90:
        logger.info(
            "   ⚠️ ALERTA: Más del 90% son ceros. Considera si esta variable aporta valor al modelo."
        )
        logger.info(
            "      (Ej: penaltyGoals_p90 es normal que sea alto en ceros, pero accurateCrosses no tanto)."
        )
    else:
        logger.info(
            "   ✅ Los niveles de ceros parecen razonables para métricas de fútbol."
        )
