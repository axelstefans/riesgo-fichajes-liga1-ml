    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        if "season" in df.columns:
            ...
            season_df = pd.DataFrame(
                {"Cantidad": season_counts, "Porcentaje": season_pct}
            )
...
def grafico_1_distribucion_minutos(df):
    ...
    ax.hist(
        df["minutesPlayed"], bins=50, color="steelblue", edgecolor="black", alpha=0.7
    )
    ax.axvline(
        df["minutesPlayed"].mean(),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f"Media: {df['minutesPlayed'].mean():.0f} min",
    )
    ax.axvline(
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=2,
        label="Umbral filtrado: 450 min",
    )

    ax.set_xlabel("Minutos Jugados", fontsize=12, fontweight="bold")
    ax.set_ylabel("Frecuencia", fontsize=12, fontweight="bold")
    ax.set_title(
        "Distribución de Minutos Jugados (Dataset Crudo)",
        fontsize=14,
        fontweight="bold",
    )
...
def grafico_2_boxplot_metricas_posicion(df):
    ...
    for idx, metrica in enumerate(metricas_disponibles):
        sns.boxplot(
            data=df_plot, x="posicion_agrupada", y=metrica, ax=axes[idx], palette="Set2"
        )
        axes[idx].set_xlabel("Posición", fontsize=11, fontweight="bold")
        axes[idx].set_ylabel(
            metrica.replace("_", " ").title(), fontsize=11, fontweight="bold"
        )
        axes[idx].set_title(
            f"Distribución de {metrica.replace('_', ' ').title()} por Posición",
            fontsize=12,
            fontweight="bold",
        )
...
def grafico_3_top_nacionalidades(df):
    ...
    if "nacionalidad_str" not in df.columns:
        logger.warning(
            "⚠️ Columna 'nacionalidad_str' no encontrada. Saltando gráfico 3."
        )
        return
...
def grafico_4_registros_temporada(df):
    ...
    ax.set_title(
        "Distribución de Registros por Temporada", fontsize=14, fontweight="bold"
    )
    ax.grid(axis="y", alpha=0.3)
...
def grafico_5_matriz_correlacion(df):
    ...
    ax.set_title(
        "Matriz de Correlación - Métricas Clave (Datos Crudos)",
        fontsize=14,
        fontweight="bold",
    )
...
def grafico_6_deteccion_outliers(df):
    ...
    if len(metricas_disponibles) == 0:
        logger.warning(
            "⚠️ No hay métricas disponibles para detección de outliers. Saltando gráfico 6."
        )
        return
    ...
    for idx, metrica in enumerate(metricas_disponibles):
        if idx < len(axes):
            ...
            axes[idx].set_title(
                f"Detección de Outliers: {metrica.replace('_', ' ').title()}",
                fontsize=12,
                fontweight="bold",
            )
            axes[idx].grid(axis="y", alpha=0.3)
    ...
    plt.suptitle(
        "Detección de Outliers en Métricas Clave",
        fontsize=16,
        fontweight="bold",
        y=1.00,
    )
