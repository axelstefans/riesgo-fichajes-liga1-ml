...
def cargar_datos():
    ...
    try:
        df = pd.read_csv(
            INPUT_FILE, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
        )
        ...
        if "pos_Delantero" in df.columns and "pos_Mediocampista" in df.columns:
            ...
            logger.info(
                "  ✅ Columna 'posicion_original' reconstruida desde dummies para EDA."
            )
        else:
...
def generar_estadisticas_descriptivas(df):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.writelines(
            f"{pos:15s}: {count:4d} ({(count / len(df) * 100):5.1f}%)\n"
...
def grafico_1_matriz_correlacion(df):
    ...
    logger.info("  Mostrando top 20 por varianza...")
    ...
    plt.title(
        "Matriz de Correlación (Top 20 Features por Varianza)", fontsize=14, pad=15
    )
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "01_matriz_correlacion_completa.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
    ...
    if pares_altos:
        ...
        logger.info("  ✅ Sin correlaciones > 0.80")
...
def grafico_2_histogramas_p90(df):
    ...
    for i, col in enumerate(cols_plot):
        ...
        axes[i].set_title(
            col.replace("_p90", "").replace("_", " ").title(), fontsize=12
        )
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "03_histogramas_features_p90.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def crear_boxplot_generico(df, metrica, titulo, archivo):
    ...
    sns.boxplot(
        ...
        x="posicion_original",
        y=metrica,
        order=ORDEN_POSICION,
        palette=COLORES_POSICION,
        showfliers=True,
        linewidth=1.5,
    )
...
def grafico_6_minutos(df):
    ...
    plt.hist(
        df["minutesPlayed"], bins=40, color="#4ECDC4", edgecolor="white", alpha=0.8
    )
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "07_distribucion_minutesPlayed.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
