...
df_crudo = pd.read_csv(
    "datos_salida/brutos/datos_brutos_merged.csv", sep=";", encoding="utf-8-sig"
)
df_final = pd.read_csv(
    "datos_salida/finales/dataset_etiquetado.csv", sep=";", encoding="utf-8-sig"
)
...
if nac_cols:
    ...
    for col in sorted(nac_cols):
        ...
        print(
            f"   {col:30s} {int(count):4d} jugadores ({pct:5.2f}%) | {zero_pct:5.1f}% ceros"
        )
...
if "nacionalidad_str" in df_crudo.columns and nac_cols:
    ...
    for nac in nacionalidades_importantes:
        ...
        if col_name in df_final.columns:
            ...
            diff_pct = (
                ((count_final - count_crudo) / count_crudo * 100)
                if count_crudo > 0
                else 0
            )
...
if "nacionalidad_str" in df_crudo.columns:
    ...
    axes[0].set_title(
        "Top 10 Nacionalidades (Dataset Crudo Original)", fontweight="bold", fontsize=14
    )
...
if nac_cols:
    nac_counts_final = {
        col.replace("nac_", ""): df_final[col].sum() for col in nac_cols
    }
    ...
    # Ordenar
    nac_counts_sorted = dict(
        sorted(nac_counts_final.items(), key=lambda x: x[1], reverse=True)
    )

    axes[1].barh(
        range(len(nac_counts_sorted)),
        list(nac_counts_sorted.values()),
        color="coral",
        edgecolor="black",
    )
    ...
    axes[1].set_title(
        "Distribución de Nacionalidades (Dataset Final One-Hot)",
        fontweight="bold",
        fontsize=14,
    )
...
if nac_cols:
    ...
    print(
        "   (Una columna es 'sparse' si tiene muchos ceros, PERO esto es NORMAL para one-hot encoding)"
    )
    ...
    for col in sorted(nac_cols):
        ...
        print(
            f"      Representa {int(jugadores_count)} jugadores ({100 - zero_pct:.1f}%)"
        )
        print(
            f"      ¿Eliminar? {'NO - Representa una nacionalidad válida' if jugadores_count > 20 else 'CONSIDERAR - Muy pocos casos'}"
        )
