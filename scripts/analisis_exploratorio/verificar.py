...
def analizar_outliers_goals(df):
    ...
    outliers_delantero = df[
        (df["posicion_agrupada"] == "Delantero") & (df["goals"] > umbral_delantero)
    ].copy()
    outliers_medio = df[
        (df["posicion_agrupada"] == "Mediocampista")
        & (df["goals"] > umbral_mediocampista)
    ].copy()
    outliers_defensa = df[
        (df["posicion_agrupada"] == "Defensa") & (df["goals"] > umbral_defensa)
    ].copy()

    print(f"\n🔴 DELANTEROS CON MÁS DE {umbral_delantero} GOLES:")
    print("-" * 100)
    if len(outliers_delantero) > 0:
        outliers_delantero["goles_por_90"] = (
            outliers_delantero["goals"] / (outliers_delantero["minutesPlayed"] / 90)
        ).round(2)
        outliers_delantero["partidos_estimados"] = (
            outliers_delantero["minutesPlayed"] / 90
        ).round(1)
        ...
        for idx, row in outliers_delantero.iterrows():
            ...
            if goles_partido > 1.0:
                print(
                    f"   ⚠️ {row['nombre_jugador']}: {row['goals']} goles en {row['partidos_estimados']:.1f} partidos (~{goles_partido:.2f} goles/90min)"
                )
                print(
                    "      → VERIFICAR: Tasa extremadamente alta. Posible duplicado o error."
                )
            else:
                print(
                    f"   ✅ {row['nombre_jugador']}: {row['goals']} goles en {row['partidos_estimados']:.1f} partidos (~{goles_partido:.2f} goles/90min)"
                )
                print("      → NORMAL: Goleador excepcional pero realista.")
    else:
    ...
    if len(outliers_medio) > 0:
        outliers_medio["goles_por_90"] = (
            outliers_medio["goals"] / (outliers_medio["minutesPlayed"] / 90)
        ).round(2)
        outliers_medio["partidos_estimados"] = (
            outliers_medio["minutesPlayed"] / 90
        ).round(1)
    ...
    if len(outliers_defensa) > 0:
        outliers_defensa["goles_por_90"] = (
            outliers_defensa["goals"] / (outliers_defensa["minutesPlayed"] / 90)
        ).round(2)
        outliers_defensa["partidos_estimados"] = (
            outliers_defensa["minutesPlayed"] / 90
        ).round(1)
    ...
    outliers_goals = pd.concat(
        [outliers_delantero, outliers_medio, outliers_defensa], ignore_index=True
    )
    if len(outliers_goals) > 0:
        outliers_goals.to_csv(
            OUTPUT_DIR / "outliers_goals.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
...
def analizar_outliers_totalshots(df):
    ...
    outliers_delantero = df[
        (df["posicion_agrupada"] == "Delantero") & (df["totalShots"] > umbral_delantero)
    ].copy()
    outliers_medio = df[
        (df["posicion_agrupada"] == "Mediocampista")
        & (df["totalShots"] > umbral_mediocampista)
    ].copy()
    outliers_defensa = df[
        (df["posicion_agrupada"] == "Defensa") & (df["totalShots"] > umbral_defensa)
    ].copy()

    print(f"\n🔴 DELANTEROS CON MÁS DE {umbral_delantero} TIROS:")
    print("-" * 100)
    if len(outliers_delantero) > 0:
        outliers_delantero["tiros_por_90"] = (
            outliers_delantero["totalShots"]
            / (outliers_delantero["minutesPlayed"] / 90)
        ).round(2)
        outliers_delantero["partidos_estimados"] = (
            outliers_delantero["minutesPlayed"] / 90
        ...
        for idx, row in outliers_delantero.iterrows():
            ...
            if tiros_partido > 5.0:
                print(
                    f"   ⚠️ {row['nombre_jugador']}: {row['totalShots']} tiros (~{tiros_partido:.2f} tiros/90min)"
                )
                print(
                    "      → ALTO pero posible en delanteros rematadores compulsivos."
                )
            else:
                print(
                    f"   ✅ {row['nombre_jugador']}: {row['totalShots']} tiros (~{tiros_partido:.2f} tiros/90min)"
                )
                print("      → NORMAL para delantero activo.")
    else:
    ...
    print("-" * 100)
    if len(outliers_medio) > 0:
        outliers_medio["tiros_por_90"] = (
            outliers_medio["totalShots"] / (outliers_medio["minutesPlayed"] / 90)
        ).round(2)
        outliers_medio["partidos_estimados"] = (
            outliers_medio["minutesPlayed"] / 90
        ).round(1)
    ...
    outliers_shots = pd.concat(
        [outliers_delantero, outliers_medio, outliers_defensa], ignore_index=True
    )
    if len(outliers_shots) > 0:
        outliers_shots.to_csv(
            OUTPUT_DIR / "outliers_totalshots.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
...
def analizar_duplicados_jugador_temporada(df):
    ...
    duplicados = duplicados[duplicados["count"] > 1].sort_values(
    ...
    if len(duplicados) > 0:
        ...
        for idx, row in duplicados.head(10).iterrows():
            ...
            if registros["club_origen"].nunique() > 1:
                print(
                    "   💡 CAUSA: Jugador cambió de club en la misma temporada → NORMAL (fichaje mid-season)"
                )
            else:
                print(
                    "   ⚠️ CAUSA: Mismo club, múltiples registros → POSIBLE ERROR DE SCRAPING"
                )
                print(
                    "   🔧 ACCIÓN: Verificar en SofaScore si son stats de diferentes competencias (Liga vs Copa)"
                )

        duplicados_full = df.merge(duplicados, on=["tm_id", "season"], how="inner")
        duplicados_full.to_csv(
            OUTPUT_DIR / "duplicados_jugador_temporada.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
...
def analizar_registros_sospechosos(df):
    ...
    if len(sospechosos_1) > 0:
        ...
        sospechosos_1.to_csv(
            OUTPUT_DIR / "registros_sospechosos_stats_cero.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
    ...
    if len(sospechosos_2) > 0:
        print(
            f"⚠️ Se encontraron {len(sospechosos_2)} registros imposibles (stats sin minutos):"
        )
        cols = [
            "tm_id",
            "ss_id",
        ...
        sospechosos_2.to_csv(
            OUTPUT_DIR / "registros_sospechosos_stats_sin_minutos.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
        print(
            f"💾 Guardado en: {OUTPUT_DIR / 'registros_sospechosos_stats_sin_minutos.csv'}"
        )
    else:
    ...
    if len(sospechosos_3) > 0:
        print(
            f"⚠️ Se encontraron {len(sospechosos_3)} registros IMPOSIBLES (más goles que tiros):"
        )
        cols = [
            "tm_id",
            "ss_id",
        ...
        sospechosos_3.to_csv(
            OUTPUT_DIR / "registros_sospechosos_goals_mayor_shots.csv",
            index=False,
            sep=";",
            encoding="utf-8-sig",
        )
        print(
            f"💾 Guardado en: {OUTPUT_DIR / 'registros_sospechosos_goals_mayor_shots.csv'}"
        )
    else:
...
def generar_links_sofascore(df_outliers):
    ...
    print("   2. Verifica cada link manualmente en SofaScore")
    print("   3. Si encuentras errores, reporta los ss_id problemáticos")
...
def main():
    ...
    print("\n📋 Archivos generados:")
    print("   - outliers_goals.csv")
    print("   - outliers_totalshots.csv")
    print("   - duplicados_jugador_temporada.csv (si aplica)")
    print("   - registros_sospechosos_*.csv (si aplica)")
    print("   - links_verificacion_sofascore.txt")
