...
from core.scraping.sofascore import (
    buscar_ss_id_jugador_exacto,
    encontrar_ids_temporada_previa,
)
...
def mapear_ids_ss(df_fichajes: pd.DataFrame) -> pd.DataFrame | None:
    ...
    if num_duplicados > 0:
        logger.info(
            f"Se eliminaron {num_duplicados} duplicados. Procesando {len(df_unicos)} jugadores únicos."
        )

    ss_ids_encontrados = []

    for _, row in tqdm(
        df_unicos.iterrows(), total=len(df_unicos), desc="Mapeando jugadores"
    ):
    ...
    logger.info(
        f"Mapeo finalizado. Se encontraron {encontrados} de {total} IDs ({porcentaje:.2f}%)."
    )
...
def encontrar_temporada_previa(
    df_mapeado: pd.DataFrame, anio_fichaje: int
) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    if df_mapeado is None or df_mapeado.empty:
        logger.warning("El DataFrame de entrada está vacío. Omitiendo búsqueda.")
        return None, None
    logger.info(
        f"Iniciando búsqueda de temporada previa. Año de fichaje ref: {anio_fichaje}"
    )
    lista_resultados = []

    for _, jugador in tqdm(
        df_mapeado.iterrows(), total=len(df_mapeado), desc="Buscando historial"
    ):
        time.sleep(random.uniform(Config.MIN_DELAY, Config.MAX_DELAY))
        ss_season_id, ss_tournament_id = encontrar_ids_temporada_previa(
            jugador["ss_id"], anio_fichaje
        )
    ...
    df_exitosos = df_final.dropna(
        subset=["ss_season_id_previo", "ss_tournament_id_previo"]
    ).copy()
    ...
    porcentaje = (encontrados / total) * 100 if total > 0 else 0
    logger.info(
        f"Búsqueda finalizada. Se encontró temp. previa para {encontrados} de {total} jugadores ({porcentaje:.2f}%)."
    )
    return (
        df_exitosos if not df_exitosos.empty else None,
        df_fallidos if not df_fallidos.empty else None,
    )
...
def recolectar_ids_temporada(config_temporada: dict) -> dict:
    ...
    if os.path.exists(output_file):
        df_existente = pd.read_csv(
            output_file, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
        )
        logger.info(
            f"El archivo para la temporada {temporada_id} ya existe. Omitiendo recolección."
        )
    ...
    if df_tm is None:
        return {"estado": "Error TM", "registros": 0}

    logger.info("Mapeando con IDs de SofaScore...")
    df_mapeado = mapear_ids_ss(df_tm)
    if df_mapeado is None:
        return {"estado": "Error Mapeo SS", "registros": 0}
    ...
    if df_exitosos is None:
        return {"estado": "Error Historial SS", "registros": 0}

    if df_fallidos is not None and not df_fallidos.empty:
        logger.warning(
            f"Guardando {len(df_fallidos)} registros fallidos para revisión manual..."
        )
    ...
    df_exitosos.to_csv(
        output_file, index=False, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
    )
...
if __name__ == "__main__":
    ...
    for temporada_name, temporada_config in Config.TEMPORADAS.items():
        ...
        temporada_config["output_file"] = (
            f"{Config.DIR_SALIDA}/fichajes_{temporada_config['tm_saison_id']}_{temporada_config['anio_fichaje']}.csv"
        )
