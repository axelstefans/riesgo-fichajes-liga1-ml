...
from pipeline_recoleccion import (
    encontrar_temporada_previa,
    mapear_ids_ss,
    obtener_fichajes_tm,
)
...
def cargar_o_crear_csv(path, funcion_crear, *args):
    if os.path.exists(path):
        logger.info(
            f"Archivo '{os.path.basename(path)}' ya existe. Cargando desde disco."
        )
        df = pd.read_csv(
            path, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING, dtype=str
        )
        if not df.empty:
            return df
        logger.warning(
            f"Archivo '{os.path.basename(path)}' existe pero está vacío. Se intentará recrearlo."
        )
    ...
    if df is not None and not df.empty:
        ...
        df.to_csv(
            path, index=False, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
        )
...
if __name__ == "__main__":
    if not Config.TEMPORADAS:
        logger.error(
            "El diccionario 'TEMPORADAS' en config.py está vacío. No hay nada que procesar."
        )
        exit(1)
    ...
    for temporada_nombre, config_temporada in Config.TEMPORADAS.items():
        logger.info(
            f"--- INICIANDO FASE 1: RECOLECCIÓN DE IDS PARA {temporada_nombre} ---"
        )

        paths = generar_paths(
            config_temporada["file_suffix"], config_temporada["tm_saison_id"]
        )
        resultado_temporada = {
        ...
        try:
            ...
            df_fichajes = cargar_o_crear_csv(
                paths["fichajes_tm"],
                obtener_fichajes_tm,
                config_temporada["tm_saison_id"],
            ...
            df_mapeado = cargar_o_crear_csv(
                paths["fichajes_mapeados"],
                mapear_ids_ss,
                df_fichajes,
                Config.DRIVER_VERSION,
            )
            ...
            if os.path.exists(paths["fichajes_final"]):
                logger.info(
                    f"Archivo final '{os.path.basename(paths['fichajes_final'])}' ya existe. Omitiendo este paso."
                )
                ...
                if df_exitosos is not None and not df_exitosos.empty:
                    ...
                    logger.warning(
                        "Archivo final cargado estaba vacío. Se forzará la recreación."
                    )
                    df_exitosos, df_fallidos = encontrar_temporada_previa(
                        df_mapeado,
                        config_temporada["anio_fichaje"],
                        Config.DRIVER_VERSION,
                    )
            else:
                df_exitosos, df_fallidos = encontrar_temporada_previa(
                    df_mapeado, config_temporada["anio_fichaje"], Config.DRIVER_VERSION
                )

            if df_exitosos is not None and not df_exitosos.empty:
                df_exitosos.to_csv(
                    paths["fichajes_final"],
                    index=False,
                    sep=Config.CSV_SEPARATOR,
                    encoding=Config.CSV_ENCODING,
                )
                logger.info(
                    f"Guardados {len(df_exitosos)} registros listos para la siguiente fase."
                )
                resultado_temporada["registros_finales"] = len(df_exitosos)
            ...
            if df_fallidos is not None and not df_fallidos.empty:
                ...
                df_fallidos.to_csv(
                    paths["fichajes_fallidos"],
                    index=False,
                    sep=Config.CSV_SEPARATOR,
                    encoding=Config.CSV_ENCODING,
                )
                logger.info(
                    f"Guardados {len(df_fallidos)} registros fallidos para revisión."
                )
                resultado_temporada["registros_fallidos"] = len(df_fallidos)

            logger.info(
                f"--- PROCESO PARA TEMPORADA {temporada_nombre} COMPLETADO ---\n"
            )

        except Exception as e:
            logger.error(
                f"Error procesando temporada {temporada_nombre}: {e}", exc_info=True
            )
    ...
    for r in resumen_temporadas:
        logger.info(
            f"{r['temporada']:<15} {r['fase_completada']:<30} {r['registros_finales']:<15} {r['registros_fallidos']:<15}"
        )
    logger.info("=" * 80)
