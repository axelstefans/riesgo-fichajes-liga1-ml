...
def verificar_dataset_final(
    csv_path: str = "datos_salida/finales/dataset_entrenamiento_final.csv",
):
    """
    ...
    if "minutesPlayed" in features:
        ...
        if abs(corr) > 0.7:
            ...
            print("         Posible leakage o proxy del target")
        elif abs(corr) > 0.4:
            print(f"      ⚠️ Correlación moderada ({corr:.3f})")
            print("         Verificar que sea del PASADO")
        else:
    ...
    if cat_cols:
        print("   ⚠️ Columnas categóricas detectadas (deberían ser numéricas):")
        for col in cat_cols:
    ...
    print(
        df_escalas.head(10)[["feature", "min", "max", "rango"]].to_string(index=False)
    )
    ...
    temporadas = sorted(
        df["season"].unique(), key=lambda s: tuple(map(int, s.split("_")))
    )
    ...
    for temp in temporadas:
        ...
        print(
            f"   {temp}: n={total:>3} | Riesgo Alto={conteo.get(1.0, 0):>3} ({pct_1:.1f}%) | Bajo={conteo.get(0.0, 0):>3}"
        )
    ...
    if high_corr:
        ...
        for pair in high_corr[:10]:  # Mostrar solo top 10
            print(
                f"      - {pair['feature_1']} <-> {pair['feature_2']}: {pair['correlacion']:.3f}"
            )
    else:
    ...
    if len(rangos_grandes) > 0 and len(rangos_pequeños) > 0:
        warnings.append(
            "⚠️ Features con escalas muy diferentes (normalización recomendada)"
        )
