...
def reconstruir_posicion(row):
    if row.get("pos_Delantero") == 1:
        return "Delantero"
    elif row.get("pos_Mediocampista") == 1:
...
def main():
    ...
    # 1. Rutas
    path_pasado = os.path.join(
        Config.DIR_SALIDA_PROCESADOS, "fichajes_preprocesados.csv"
    )
    path_futuro = os.path.join(
        Config.DIR_SALIDA_PROCESADOS, "rendimiento_posterior_preprocesado.csv"
    )
    path_final = os.path.join(
        Config.DIR_SALIDA_FINALES, "dataset_entrenamiento_final.csv"
    )

    # 2. Cargar
    df_pasado = pd.read_csv(
        path_pasado, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
    )
    df_futuro = pd.read_csv(
        path_futuro, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING
    )
    ...
    logger.info(
        f"⚠️  Excluidas temporadas < {TEMPORADA_INICIAL_MINIMA}: {registros_excluidos} registros"
    )
    ...
    df_merged["season"] = pd.Categorical(
        df_merged["season"], categories=orden_temporal, ordered=True
    )
    ...
    # REGLA A: INACTIVIDAD
    col_minutos_fut = (
        "minutesPlayed_futuro"
        if "minutesPlayed_futuro" in df_merged.columns
        else "minutesPlayed"
    )
    ...
    if not df_activos.empty:
        ...
        for pos in ["Defensa", "Mediocampista", "Delantero"]:
            ...
            # Etiquetar
            es_riesgo_alto = (
                df_merged.loc[indices_pos, "score_futuro"] <= umbral_expandido
            )
            df_merged.loc[indices_pos, "riesgo_fichaje"] = es_riesgo_alto.astype(int)

            logger.info(
                f"   ✅ {pos}: {mask_pos_activos.sum()} etiquetados (expanding)"
            )
    ...
    # 6. Guardar
    df_final.to_csv(
        path_final, sep=Config.CSV_SEPARATOR, encoding=Config.CSV_ENCODING, index=False
    )
    ...
    logger.info(
        f"📊 Total: {len(df_final)} (excluidos {registros_excluidos} de 2017_2018)"
    )
