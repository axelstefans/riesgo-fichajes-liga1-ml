...
def cargar_datos():
    ...
    logger.info(
        f"   - Bajo Riesgo (0): {balance.get(0, 0)} ({balance.get(0, 0) / len(df) * 100:.1f}%)"
    )
    logger.info(
        f"   - Alto Riesgo (1): {balance.get(1, 0)} ({balance.get(1, 0) / len(df) * 100:.1f}%)"
    )
...
def generar_reporte_estadistico(df):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Ratio observaciones/features: {len(df) / len(numeric_features):.2f}\n\n"
        )
        ...
        balance = df["riesgo_fichaje"].value_counts()
        f.write(
            f"Bajo Riesgo (0): {balance.get(0, 0):4d} ({balance.get(0, 0) / len(df) * 100:5.2f}%)\n"
        )
        f.write(
            f"Alto Riesgo (1): {balance.get(1, 0):4d} ({balance.get(1, 0) / len(df) * 100:5.2f}%)\n\n"
        )

        f.write("📋 LISTA DE FEATURES PREDICTORAS\n")
        f.write("-" * 100 + "\n")
        f.writelines(
        ...
        f.write("📊 ESTADÍSTICAS DESCRIPTIVAS POR CLASE\n")
        f.write("-" * 100 + "\n")
        f.write("\nBAJO RIESGO (0):\n")
...
def grafico_3_barras_contexto(df):
    ...
    crosstab = (
        pd.crosstab(
            df["contexto_equipo_top"], df["riesgo_fichaje"], normalize="columns"
        )
        * 100
    )
...
def grafico_6_matriz_correlacion(df):
    ...
    correlaciones = (
        df[numeric_features + ["riesgo_fichaje"]].corr()["riesgo_fichaje"].abs()
    )
    correlaciones = correlaciones.sort_values(ascending=False)

    top_12_features = correlaciones[1:13].index.tolist()

    logger.info("   Top 12 features más correlacionadas con el target:")
...
def main():
    ...
    try:
        ...
        logger.info("📊 Total de gráficos generados: 6")
        logger.info("")
