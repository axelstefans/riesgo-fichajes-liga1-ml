    ...
    try:
        ...
        if not archivos_entrada:
            raise FileNotFoundError(
                f"No se encontraron archivos de entrada en {dir_entrada}."
            )
        df_contexto_list = [
            pd.read_csv(f, sep=Config.CSV_SEPARATOR, dtype={"tm_id": str})
            for f in archivos_entrada
        ]
        df_contexto_unificado = pd.concat(df_contexto_list, ignore_index=True)
        df_contexto_to_merge = df_contexto_unificado[
            ["tm_id", "ss_season_id", "club_destino"]
        ].copy()
        ...
        df_contexto_to_merge["ss_season_id"] = df_contexto_to_merge[
            "ss_season_id"
        ].astype(str)

        df_merged = pd.merge(
            df_bruto, df_contexto_to_merge, on=["tm_id", "ss_season_id"], how="left"
        )
        ...
        logger.info(
            f"Limpieza POST-agregación: Se eliminaron {eliminados_umbral} registros con menos de 450 minutos."
        )
        ...
        metadata_cols = [
            c
            for c in ["tm_id", "ss_id", "nombre_jugador", "season"]
            if c in df_procesado.columns
        ]
        feature_cols = sorted(
            [c for c in df_procesado.columns if c not in metadata_cols]
        )
        df_reordenado = df_procesado[metadata_cols + feature_cols]

        logger.info("Ordenando el dataset final por 'season' y 'tm_id'...")
        df_final = df_reordenado.sort_values(
            by=["season", "tm_id"], ascending=[True, True]
        ...
        logger.info(
            f"📊 Dimensiones finales: {df_final.shape[0]} filas x {df_final.shape[1]} columnas"
        )
