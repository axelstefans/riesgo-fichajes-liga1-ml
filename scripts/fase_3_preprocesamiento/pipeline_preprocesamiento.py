...
def cargar_y_unificar_datos_brutos(directorio_brutos: Path) -> pd.DataFrame:
    ...
    if not directorio_brutos.is_dir():
        raise FileNotFoundError(
            f"El directorio de datos brutos no existe: {directorio_brutos}"
        )

    archivos_csv = sorted(directorio_brutos.glob("*.csv"))
    if not archivos_csv:
        raise FileNotFoundError(
            f"No se encontraron archivos CSV en {directorio_brutos}"
        )
...
def corregir_anomalia_started_appearances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if num_anomalias > 0:
        logger.warning(
            f"  - Se detectaron {num_anomalias} registros con 'started' > 'appearances'. Procediendo a corregir."
        )

        started_original = df.loc[filtro_anomalia, "started"]
        appearances_original = df.loc[filtro_anomalia, "appearances"]

        df.loc[filtro_anomalia, "appearances"] = started_original.combine(
            appearances_original, max
        )
        df.loc[filtro_anomalia, "started"] = started_original.combine(
            appearances_original, min
        )
...
def agregar_stats_por_temporada(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_agregado = (
        df.groupby(["tm_id", "season"])
        .agg(
            {
                **{col: "sum" for col in columnas_sumar_existentes},
                **{
                    col: "first"
                    for col in columnas_meta_existentes
                    if col not in ["tm_id", "season"]
                },
            }
        )
        .reset_index()
    ...
    logger.info(
        f"Agregación completada. Registros únicos por jugador-temporada: {len(df_agregado)}"
    )
    return df_agregado
...
def limpiar_filas(df: pd.DataFrame, minutos_threshold: int = 450) -> pd.DataFrame:
    ...
    logger.info(
        f"Iniciando limpieza de filas. Registros iniciales: {registros_iniciales}"
    )
    ...
    registros_antes = len(df_limpio)
    filtro_conversion_absurda = (df_limpio["goals"] > df_limpio["totalShots"]) | (
    ...
    logger.info(
        f"  - Se eliminaron {eliminados_conversion} filas con 'goals > totalShots' (error de scraping)."
    )
    ...
    logger.info(
        f"  - Se eliminaron {eliminados_umbral} filas con menos de {minutos_threshold} minutos."
    )

    registros_finales = len(df_limpio)
    logger.info(
        f"Limpieza de filas completada. Registros restantes: {registros_finales}"
    )
    return df_limpio
...
def seleccionar_features_finales(df: pd.DataFrame) -> pd.DataFrame:
    ...
    features_baja_relevancia_eliminadas = [
        col for col in FEATURES_BAJA_RELEVANCIA if col in df.columns
    ]

    logger.info(
        f"  - Columnas originales eliminadas: {len(COLUMNAS_NUMERICAS_ORIGINALES + COLUMNAS_CONTEXTUALES_ORIGINALES)}"
    )
    logger.info(
        f"  - Features redundantes eliminadas: {len(COLUMNAS_ENGINEERED_REDUNDANTES)}"
    )
    logger.info(
        f"  - Features de baja relevancia eliminadas: {len(features_baja_relevancia_eliminadas)}"
    )
    logger.info(
        f"  - Features multicolineales eliminadas: {len(FEATURES_MULTICOLINEALES)}"
    )
    logger.info(
        f"  - Features incompatibles (Cobertura 2da División) eliminadas: {len(COLUMNAS_COMPLEJAS_INCOMPATIBLES)}"
    )

    if "pos_Delantero" in df_final.columns and "pos_Mediocampista" in df_final.columns:
        logger.info(
            "  ✅ Dummies de posición (pos_Delantero, pos_Mediocampista) mantenidas para modelo."
        )
    else:
