...
def grafico_convergencia_arboles(X_train, y_train, X_val, y_val, output_dir):
    ...
    for i, n in enumerate(n_trees_range):
        ...
        if (i + 1) % 5 == 0:
            print(
                f"   Progreso: {i + 1}/{len(n_trees_range)} | "
    ...
    ax.set_title(
        "Convergencia de RandomForest: F1-Score vs Número de Árboles",
        fontsize=15,
        pad=20,
        fontweight="bold",
    )
