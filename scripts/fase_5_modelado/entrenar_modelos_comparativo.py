...
EXCLUDE_COLS_BASE = {"tm_id", "ss_id", "nombre_jugador", "season", "riesgo_fichaje"}
...
def seleccionar_features(df: pd.DataFrame, target: str) -> list[str]:
    ...
    if "pos_Delantero" not in feats or "pos_Mediocampista" not in feats:
        ...
        logger.info(
            "✅ Dummies de posición presentes: pos_Delantero, pos_Mediocampista"
        )
...
def plot_per_fold_diagnostics(
    y_true: np.ndarray, y_proba: np.ndarray, title_prefix: str, outdir: Path
) -> None:
    ...
    try:
        ...
        ax.set_title(
            f"{modelo_nombre}\nMatriz de Confusión (Test={title_prefix.split('_')[-1]})",
            fontsize=13,
        )
        plt.tight_layout()
    ...
    try:
        ...
        ax.set_title(
            f"{modelo_nombre}\nCurva ROC (Test={title_prefix.split('_')[-1]})",
            fontsize=13,
        )
    ...
    try:
        ...
        ax.set_title(
            f"{modelo_nombre}\nCurva Precision-Recall (Test={title_prefix.split('_')[-1]})",
            fontsize=13,
        )
...
def plot_feature_importance(
    modelo_entrenado, feature_names: list, model_name: str, outdir: Path
):
    ...
    if importances is None:
        logger.info(
            f"   (Skipping Feature Importance: {model_name} no tiene importances)"
        )
        return
    ...
    sns.barplot(
        x="importance", y="feature", data=df_imp, orient="h", ax=ax, color=color
    )
...
def plot_summary_charts(
    df_summary: pd.DataFrame, df_last_season: pd.DataFrame, outdir: Path
):
    ...
    try:
        ...
        ax.set_title(
            "Rendimiento Promedio (Walk-Forward CV, 5 Folds)", fontsize=14, pad=15
        )
    ...
    try:
        ...
        ax.set_title(
            f"Rendimiento en Test Futuro (Test={test_season})", fontsize=14, pad=15
        )
...
def evaluar_modelo_walkforward(
    modelo_nombre: str,
    modelo,
    df: pd.DataFrame,
    feats: list[str],
    ...
    for train_seasons, test_season in folds:
        ...
        if test_season == ultima_temporada_test:
            logger.info(
                f"   -> Generando gráficos para el fold final (Test={test_season})..."
            )
    ...
    y_train_full = (
        df[df["season"].isin(temporadas_ordenadas[:-1])][target].astype(int).values
    )

    logger.info(
        f"   Entrenando en {len(X_train_full)} registros ({len(temporadas_ordenadas) - 1} temporadas)"
    )
...
def resumir_resultados_por_modelo(resultados_por_modelo: dict, outdir: Path):
    ...
    todas = pd.concat(
        [df for _, df in resultados_por_modelo.values()], ignore_index=True
    )
    if todas.empty:
...
def main():
    ...
    logger.info(
        f"📂 Dataset: {len(df)} filas, {df.shape[1]} columnas | {Path(args.csv).name}"
    )
    ...
    temporadas = sorted(
        df["season"].unique().tolist(), key=lambda s: tuple(map(int, s.split("_")))
    )
