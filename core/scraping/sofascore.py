...
def _hacer_peticion_segura(url, descripcion):
    ...
    response = requests.get(
        url, headers=HEADERS, impersonate=IMPERSONATE_VER, timeout=15
    )
...
def get_player_statistics_by_tournament(
    ss_player_id: str, ss_season_id: str, ss_tournament_id: str
) -> dict | None:
    ...
    if not data or "statistics" not in data:
        logger.warning(
            f"No se encontraron estadísticas en SofaScore para SS_ID {ss_player_id}, Season {ss_season_id}."
        )
        return None
        ...
        try:
            ...
            for result in data.get("results", []):
                if result.get("type") == "player":
                    ...
                    if player_id and _normalize_string(
                        partes_nombre[-1]
                    ) in _normalize_string(nombre_sofascore):
...
def encontrar_ids_temporada_previa(ss_player_id: str, anio_fichaje: int):
    try:
        historial_url = (
            f"https://api.sofascore.com/api/v1/player/{ss_player_id}/statistics/seasons"
        )
        ...
        for tournament_season_group in data.get("uniqueTournamentSeasons", []):
            ...
            for season in tournament_season_group.get("seasons", []):
                ...
                if (
                    year_field == anio_completo_previo
                    or year_field == anio_corto_previo
                ):
                    ...
                    stats_data = _hacer_peticion_segura(
                        stats_url, f"Stats T:{tournament_id} S:{season_id}"
                    )
                    if not stats_data:
                        continue
                    ...
                    if appearances > 0:
                        candidatos_temporada_previa.append(
                            {
        ...
        if not candidatos_temporada_previa:
            return None, None
        mejor_candidato = max(
            candidatos_temporada_previa, key=lambda x: x["appearances"]
        )
...
def _descargar_imagen_segura(url):
    ...
    try:
        ...
        response = requests.get(
            url, headers=HEADERS, impersonate=IMPERSONATE_VER, timeout=10
        )
        if response.status_code == 200:
...
def obtener_stats_sofascore(player_id):
    ...
    if data_seasons and "uniqueTournamentSeasons" in data_seasons:
        for t in data_seasons["uniqueTournamentSeasons"]:
            ...
            u_cat = (
                t.get("uniqueTournament", {}).get("category", {}).get("name", "Unknown")
            )
    ...
    for anio in sorted(anios_map.keys(), reverse=True):
        ...
        for t in lista_torneos:
            ...
            if d:
                ...
                if s.get("minutesPlayed", 0) > 0:
                    print(
                        f"      ✅ {t['name']} ({t['category']}): {s['minutesPlayed']} min"
                    )
        ...
        if not stats_recolectadas:
            continue

        # 4. FILTRO DE LIGAS DOMÉSTICAS
        torneos_locales = [
            s
            for s in stats_recolectadas
            if s["src_category"] not in CATEGORIAS_INTERNACIONALES
        ]
        ...
        if torneos_locales:
            ...
            if pais_ref in PAISES_SUMA:
                ...
                print(
                    f"      🏆 Seleccionado: {mejor_torneo.get('src_tournament')} ({mejor_torneo.get('minutesPlayed')} min)"
                )
                stats_finales = mejor_torneo

            return {"stats": stats_finales, "team": team_data, "profile": perfil}

        else:
            print("      ⚠️ Solo torneos internacionales. Guardando backup...")
            mejor_inter = max(
                stats_recolectadas, key=lambda x: x.get("minutesPlayed", 0)
            )
            if not backup_internacional or mejor_inter.get(
                "minutesPlayed", 0
            ) > backup_internacional["stats"].get("minutesPlayed", 0):
                backup_internacional = {
                    "stats": mejor_inter,
                    "team": team_data,
                    "profile": perfil,
                }
