...
def crear_features_numericas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in numeric_inputs:
        if col in df_featured.columns:
            df_featured[col] = pd.to_numeric(df_featured[col], errors="coerce").fillna(
                0
            )
        else:
            df_featured[col] = 0
...
def crear_features_contextuales(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Dummies Posición
    df_context["posicion_agrupada"] = (
        df_context["posicion"].map(POS_MAP).fillna("Defensa")
    )
    df_context = pd.get_dummies(
        df_context, columns=["posicion_agrupada"], prefix="pos", dtype=int
    )
    ...
    df_context = pd.get_dummies(
        df_context, columns=["nacionalidad_agrupada"], prefix="nac", dtype=int
    )
    for nac in [
        "nac_Perú",
        "nac_Argentina",
    ...
    df_context["contexto_equipo_top"] = (
        df_context["club_destino"].isin(Config.EQUIPOS_TOP4).astype(int)
    )
    df_context["proviene_liga_extranjera"] = (
