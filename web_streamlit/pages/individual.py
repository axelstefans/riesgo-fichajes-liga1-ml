...
# --- IMPORTS DEL SISTEMA ---
from utils.model_io import clasificar_riesgo, load_prediction_assets, predict_proba_safe
...
def _load_artifacts():
    try:
        ...
        # load_prediction_assets devuelve: model, metadata, lista_ejemplos
        # Antes asignabas la lista a 'explainer', lo que causaba el error.
    ...
    if st.session_state.get("explainer_loaded", False) and explainer:
        ...
        try:
            ...
            for i in indices:
                ...
                if "_p90" in feat_raw:
                    val_str += " p/90"
                elif "Percentage" in feat_raw:
                    val_str += "%"
                elif "edad" in feat_raw:
                    val_str += " años"
                elif "contexto" in feat_raw or "pos_" in feat_raw:
                    val_str = "Sí" if feat_val > 0.5 else "No"
            ...
            st.write("")
            with st.expander(
                "📉 Ver gráfico técnico detallado (SHAP Waterfall)", expanded=False
            ):
                st.caption(
                    "Desglose matemático de las variables que más impactaron la decisión:"
                )
                X_grafico = X.copy()
                ...
                explanation = shap.Explanation(
                    values=vals,
                    base_values=base,
                    data=X_grafico.iloc[0],
                    feature_names=X_grafico.columns,
                )
...
def render():
    ...
    st.markdown(
        "Evalúa el riesgo de contratar un jugador basándote en su **rendimiento real reciente**."
    )
            ...
            if st.button("🔍 Buscar", use_container_width=True):
                ...
                if len(search_query) > 2:
                    with st.spinner("Conectando..."):
                        st.session_state.search_results = buscar_jugador_sofascore(
                            search_query
                        )

        if st.session_state.search_results:
            opts = {
                f"{r['name']} ({r.get('team', {}).get('name', 'Libre')})": r
    ...
    if datos:
        ...
        with st.container(border=True):
            ...
            with col_img:
                if datos.get("imagen_url"):
                    st.image(datos["imagen_url"], width=200)
                else:
                    st.write("👤")
            with col_info:
                st.text_input(
                    "Nombre del jugador",
                    value=datos.get("nombre_jugador", "Jugador"),
                    disabled=True,
                )
                r1, r2 = st.columns(2)
                r1.text_input("Edad", value=datos.get("edad"), disabled=True)
                r1.text_input(
                    "Nacionalidad", value=datos.get("nacionalidad_str"), disabled=True
                )
                r2.text_input(
                    "Posición Original", value=datos.get("posicion"), disabled=True
                )
                r2.text_input(
                    "Club Actual", value=datos.get("club_origen"), disabled=True
                )
        ...
        with st.form("form_analisis"):
            ...
            t_part, t_of, t_def = st.tabs(
                ["⏱️ Participación", "⚽ Ofensiva", "🛡️ Defensa"]
            )

            with t_part:
                c1, c2, c3 = st.columns(3)
                with c1:
                    minutesPlayed = st.number_input(
                        "Minutos jugados *", 0, 5000, int(datos.get("minutesPlayed", 0))
                    )
                    appearances = st.number_input(
                        "Partidos jugados *", 0, 60, int(datos.get("appearances", 0))
                    )
                with c2:
                    yellowCards = st.number_input(
                        "Tarjetas amarillas", 0, 50, int(datos.get("yellowCards", 0))
                    )
                with c3:
                    fouls = st.number_input(
                        "Faltas cometidas", 0, 300, int(datos.get("fouls", 0))
                    )
                    wasFouled = st.number_input(
                        "Faltas recibidas", 0, 300, int(datos.get("wasFouled", 0))
                    )
            ...
            with t_of:
                ...
                with c1:
                    ...
                    assists = st.number_input(
                        "Asistencias", 0, 100, int(datos.get("assists", 0))
                    )
                    penaltyGoals = st.number_input(
                        "Goles penal", 0, 50, int(datos.get("penaltyGoals", 0))
                    )
                    penaltiesTaken = st.number_input(
                        "Penales lanzados", 0, 50, int(datos.get("penaltiesTaken", 0))
                    )
                with c2:
                    totalShots = st.number_input(
                        "Tiros totales", 0, 300, int(datos.get("totalShots", 0))
                    )
                    shotsOnTarget = st.number_input(
                        "Tiros a puerta", 0, 200, int(datos.get("shotsOnTarget", 0))
                    )
                    shotsOffTarget = st.number_input(
                        "Tiros fuera", 0, 200, int(datos.get("shotsOffTarget", 0))
                    )
                    blockedShots = st.number_input(
                        "Tiros bloqueados", 0, 100, int(datos.get("blockedShots", 0))
                    )
                with c3:
                    keyPasses = st.number_input(
                        "Pases clave", 0, 150, int(datos.get("keyPasses", 0))
                    )
                    successfulDribbles = st.number_input(
                        "Regates exitosos",
                        0,
                        300,
                        int(datos.get("successfulDribbles", 0)),
                    )
                    offsides = st.number_input(
                        "Offsides", 0, 150, int(datos.get("offsides", 0))
                    )

            with t_def:
                c1, c2, c3 = st.columns(3)
                with c1:
                    totalPasses = st.number_input(
                        "Pases totales", 0, 3000, int(datos.get("totalPasses", 0))
                    )
                    accuratePasses = st.number_input(
                        "Pases acertados", 0, 3000, int(datos.get("accuratePasses", 0))
                    )
                    accurateFinalThirdPasses = st.number_input(
                        "Pases ult. tercio",
                        0,
                        1000,
                        int(datos.get("accurateFinalThirdPasses", 0)),
                    )
                with c2:
                    accurateCrosses = st.number_input(
                        "Centros acertados",
                        0,
                        500,
                        int(datos.get("accurateCrosses", 0)),
                    )
                    accurateLongBalls = st.number_input(
                        "Balones largos ok",
                        0,
                        500,
                        int(datos.get("accurateLongBalls", 0)),
                    )
                    clearances = st.number_input(
                        "Despejes", 0, 500, int(datos.get("clearances", 0))
                    )
                with c3:
                    aerialDuelsWon = st.number_input(
                        "Duelos aéreos", 0, 300, int(datos.get("aerialDuelsWon", 0))
                    )
                    dribbledPast = st.number_input(
                        "Veces regateado", 0, 200, int(datos.get("dribbledPast", 0))
                    )

            st.write("")
            submitted = st.form_submit_button(
                "🚀 EJECUTAR ANÁLISIS DE RIESGO", use_container_width=True
            )

        if submitted:
            if not club_destino:
                st.error("⚠️ Falta Club Destino")
                return
            if minutesPlayed == 0:
                st.error("⚠️ Minutos = 0")
                return

            suma_tiros = shotsOnTarget + shotsOffTarget + blockedShots
            totalShots = max(totalShots, suma_tiros)

            raw_data = {
                "edad": datos["edad"],
                "posicion": datos["posicion"],  # Posición Original (Sin simulación)
            ...
            try:
                ...
                _mostrar_resultados(
                    datos.get("nombre_jugador"), proba, explainer, X, metadata, raw_data
                )
            except Exception as e:
                st.error(f"Error: {e}")
        ...
        with st.container(border=True):
            st.info(
                "👈 **Comienza aquí:** Escribe el nombre de un jugador en el buscador (Paso 1)."
            )
