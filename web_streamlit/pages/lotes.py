...
RAW_REQUIRED = [
    ...
    "balones_perdidos",
    "posesion_perdida",
    "toques",  # 'toques' es útil p/ toques_balon_90 si lo tienes
    # Disciplina
    "tarjetas_amarillas",
    "tarjetas_rojas",
    "faltas_cometidas",
...
def derive_features_from_raw(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["contribuciones_gol_90"] = _per90(
        df["goles"].fillna(0) + df["asistencias"].fillna(0), m
    )
    ...
    df["tarjetas_90"] = _per90(
        df["tarjetas_amarillas"].fillna(0) + df["tarjetas_rojas"].fillna(0), m
    )

    # Porcentajes
    df["titularidad_pct"] = _pct(
        df["partidos_titular"].fillna(0), df["partidos_totales"].fillna(0)
    )
    df["duelos_ganados_pct"] = _pct(
        df["duelos_ganados"].fillna(0), df["duelos_totales"].fillna(0)
    )
    df["pases_acertados_pct"] = _pct(
        df["pases_completados"].fillna(0), df["pases_totales"].fillna(0)
    )
    ...
    df["minutos_jugados_total"] = pd.to_numeric(
        df["minutos_jugados_total"], errors="coerce"
    )

    # Columnas finales candidatas (sin targets ni fugas)
    derived_cols = [
        "edad",
        "contribuciones_gol_90",
        "pases_clave_90",
        "tiros_puerta_90",
        "tackles_90",
        "faltas_cometidas_90",
...
def build_X_for_model(
    df_derived: pd.DataFrame, feature_order: list[str]
) -> pd.DataFrame:
...
def render():
    ...
    st.caption(
        "Sube un Excel con datos crudos de jugadores; la app derivará las métricas por-90 y predecirá el **riesgo de fichaje**."
    )
    ...
    if dropped > 0:
        st.warning(
            f"{dropped} jugadores fueron omitidos por tener < {MIN_MINUTES} minutos."
        )
