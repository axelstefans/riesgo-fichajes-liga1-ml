...
def test_existencia_y_carga_assets():
    ...
    try:
        ...
        pytest.fail(
            "No se encontraron los archivos .joblib o .json en la carpeta assets"
        )
...
def test_feature_engineering_logica():
    ...
    assert df_res.iloc[0]["goals_p90"] == 0.2, (
        "El cálculo de p90 es incorrecto (2/10 part = 0.2)"
    )
    assert df_res.iloc[0]["accuratePassesPercentage"] == 80.0, (
        "El cálculo de porcentaje es incorrecto"
    )
